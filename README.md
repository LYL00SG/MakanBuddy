...
- **Data:** a hand-curated local `data/places.json` of ~216 real, well-known Singapore food
...
- **Requires connectivity and API credit.** Every recommendation makes an OpenAI web-search call,
  ...
  offline local guide (~216 hand-curated real places), which is smaller and not live-updated.
...
data/places.json  # Hand-curated ~216-place real dataset (offline fallback)
